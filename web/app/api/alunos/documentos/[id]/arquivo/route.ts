...
export async function GET(_request: Request, { params }: RouteProps) {
  ...
  try {
    const resultado = await get(documento.chaveArmazenamento, {
      access: "private",
    });
    ...
    const nome = nomeSeguro(documento.nomeArquivoOriginal ?? "documento");

    return new Response(resultado.stream, {
      headers: {
        "Content-Type":
          documento.mimeType || resultado.blob.contentType || "application/octet-stream",
    ...
    console.error("Erro ao abrir documento do aluno:", error);
